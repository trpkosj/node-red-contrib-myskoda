...
class SkodaLibrary {
    ...
    _buildCookieHeader(cookies) {
        // Deduplicate cookies by name, keeping the last (most recent) value
    ...
    async _followRedirectChain(startLocation, cookies, UA) {
        // Strategy 1: Manual redirect following (works on most environments)
        try {
            const result = await this._manualRedirectFollow(startLocation, cookies, UA);
            return result;
        } catch (manualErr) {
            this.node.log(`Primary redirect method failed, trying fallback: ${manualErr.message}`);
        }

        // Strategy 2: Let axios follow redirects automatically
        // When it hits myskoda:// it will throw - we catch that and extract the URL
        try {
            let redirectUrl = startLocation;
            if (redirectUrl.startsWith("/")) {
                redirectUrl = `${BASE_URL_IDENT}${redirectUrl}`;
            }
            const fallbackResponse = await axios.get(redirectUrl, {
                maxRedirects: 15,
                validateStatus: () => true,
                headers: {
                    "User-Agent": UA,
                    "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8",
                    "Cookie": this._buildCookieHeader(cookies),
                },
                // beforeRedirect captures each redirect step
                beforeRedirect: (options, { headers: responseHeaders }) => {
                    // Collect cookies from intermediate redirects
                    if (responseHeaders["set-cookie"]) {
                        const newCookies = Array.isArray(responseHeaders["set-cookie"])
                            ? responseHeaders["set-cookie"]
                            : [responseHeaders["set-cookie"]];
                        cookies = cookies.concat(newCookies);
                        options.headers["Cookie"] = this._buildCookieHeader(cookies);
                    }
                    const nextUrl = options.href || options.url;
                    if (nextUrl && nextUrl.startsWith("myskoda://")) {
                        // Throw with the URL so we can extract it
                        const err = new Error("__MYSKODA_REDIRECT__");
                        err.myskodaUrl = nextUrl;
                        throw err;
                    }
                    if (nextUrl && nextUrl.includes("terms-and-conditions")) {
                        throw new Error("You must accept the Terms and Conditions in the MySkoda app first.");
                    }
                    if (nextUrl && nextUrl.includes("consent/marketing")) {
                        throw new Error("You must handle the marketing consent in the MySkoda app first.");
                    }
                },
            });
            // Axios followed all HTTP redirects and got a final response
            // Check if the response body contains myskoda:// URL (JS/meta redirect)
            const responseBody = typeof fallbackResponse.data === 'string' ? fallbackResponse.data : JSON.stringify(fallbackResponse.data);

            // Search for myskoda:// in the response body
            const myskodaMatch = responseBody.match(/(myskoda:\/\/[^\s"'<>]+)/);
            if (myskodaMatch) {
                return myskodaMatch[1];
            }

            // Check for an HTML-based redirect in the final response
            const htmlRedirect = this.extractHtmlRedirect(responseBody, redirectUrl);
            if (htmlRedirect && htmlRedirect.startsWith("myskoda://")) {
                return htmlRedirect;
            }

            // Check response URL (final URL after redirects) for auth code
            const finalUrl = fallbackResponse.request && fallbackResponse.request.res && fallbackResponse.request.res.responseUrl;
            if (finalUrl && finalUrl.startsWith("myskoda://")) {
                return finalUrl;
            }

            // Check if we landed back on the login page (wrong credentials)
            if (responseBody.includes('loginAuthenticate') || responseBody.includes('login/authenticate')) {
                throw new Error("Authentication failed. Please check your email and password.");
            }
            if (responseBody.includes('loginIdentifier') || responseBody.includes('login/identifier')) {
                throw new Error("Authentication failed. Please check your email address.");
            }

            throw new Error(`Redirect following did not reach myskoda:// URL. Final status: ${fallbackResponse.status}`);
        } catch (fallbackErr) {
            if (fallbackErr.myskodaUrl) {
                return fallbackErr.myskodaUrl;
            }
            // Check if the error response has a location header pointing to myskoda://
            if (fallbackErr.response && fallbackErr.response.headers && fallbackErr.response.headers["location"]) {
                const loc = fallbackErr.response.headers["location"];
                if (loc.startsWith("myskoda://")) {
                    return loc;
                }
            }
            // Check if error message contains the myskoda URL (some axios versions include it)
            if (fallbackErr.message && fallbackErr.message.includes("myskoda://")) {
                const match = fallbackErr.message.match(/(myskoda:\/\/[^\s"']+)/);
                if (match) {
                    return match[1];
                }
            }
    ...
    async _manualRedirectFollow(startLocation, cookies, UA) {
        ...
        for (let i = 0; i < maxRedirects; i++) {
            ...
            if (!location) {
                // Check if we landed back on the login page (wrong credentials)
                const body = typeof rResponse.data === 'string' ? rResponse.data : '';
                if (body.includes('loginAuthenticate') || body.includes('login/authenticate')) {
                    throw new Error("Authentication failed. Please check your email and password.");
                }
                if (body.includes('loginIdentifier') || body.includes('login/identifier')) {
                    throw new Error("Authentication failed. Please check your email address.");
                }
                // Try to extract redirect from HTML body (meta refresh, JS redirect, form action)
                const htmlRedirect = this.extractHtmlRedirect(rResponse.data, redirectUrl);
                if (htmlRedirect) {
                    location = htmlRedirect;
                } else {
    ...
    extractHtmlRedirect(html, baseUrl) {
        ...
        const metaMatch = html.match(/<meta[^>]+http-equiv\s*=\s*["']?refresh["']?[^>]+content\s*=\s*["'][^"']*url\s*=\s*([^"'\s>]+)/i)
            || html.match(/content\s*=\s*["'][^"']*url\s*=\s*([^"'\s>]+)[^>]*http-equiv\s*=\s*["']?refresh["']?/i);
        if (metaMatch) return metaMatch[1];

        // Check for JavaScript redirects: window.location.href = "..." or window.location = "..."
        const jsMatch = html.match(/window\.location(?:\.href)?\s*=\s*["']([^"']+)["']/i)
            || html.match(/window\.location\.replace\s*\(\s*["']([^"']+)["']\s*\)/i);
        if (jsMatch) return jsMatch[1];

        // Check for auto-submit form with action URL
        const formMatch = html.match(/<form[^>]+action\s*=\s*["']([^"']+)["'][^>]*>/i);
        if (formMatch && html.match(/\.submit\s*\(\s*\)/i)) {
            return formMatch[1];
        }
    ...
    async followRedirects(url, method, body, contentType, existingCookies) {
        ...
        for (let i = 0; i < maxRedirects; i++) {
            ...
            if (cookies.length > 0) {
                headers["Cookie"] = this._buildCookieHeader(cookies);
            }
            if (contentType) {
    ...
    async login(email, password) {
        ...
        location = await this._followRedirectChain(location, cookies, UA);
    ...
    async apiPost(path, data = null) {
        const url = `${BASE_URL_SKODA}/api${path}`;
        if (data) this.node.log(`POST ${url} body: ${JSON.stringify(data)}`);
        try {
            const response = await axios.post(url, data, { headers: this.getHeaders() });
            return response.data;
        } catch (err) {
            if (err.response) {
                this.node.error(`POST ${url} failed ${err.response.status}: ${JSON.stringify(err.response.data)}`);
            }
            throw err;
        }
    }
